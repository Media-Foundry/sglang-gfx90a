"""Wide C4, C16 x 16K real-code service ABBA. Default-off; keep 1M KV."""
import argparse
import hashlib
...
manifest=json.loads((fixtures/'inputs16k/prefill.json').read_text())
...
assert validation['manifest_sha256']==hashlib.sha256((fixtures/'inputs16k/prefill.json').read_bytes()).hexdigest()
assert len(manifest['requests'])==16
assert sum(len(x['input_ids']) for x in manifest['requests'])==262141
...
state=life.start('P16-wide16k-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-wide16k-'+args.arm+'.server-info.json')).read_text())
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'wide16k-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
    ...
    for rep in range(2):
        ...
        rids=[f'wide16k-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'wide16k-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
