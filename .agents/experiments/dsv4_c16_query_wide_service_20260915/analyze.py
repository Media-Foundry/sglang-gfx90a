...
for arm in arms:
    ...
    assert read(arm+'/plan.json')['mix_group_size']==8
    assert read(arm+'/plan.json')['wide']==(arm=='B')
    if arm!='B': assert b'prefill wide-query-reuse selected' not in raw
    for rank in range(8):
        if arm=='B':
    ...
    for progress in read(arm+'/progress.json'):
        ...
        for r in data['rounds']:
            assert r['total_prompt_tokens']==262141 and r['completion_lengths']==[1]*16
    ...
    for rep in range(2):
        ...
        rids=[f'wide16k-{arm}-{rep}-{i}' for i in range(16)];assert set(by_id)==set(rids)
        ids=[]
...
result=dict(legs=legs,quality=quality,sources=sources,first_quality_wave_cross_arm_exact=comparisons,
    ...
    scope='Original V4 TP8/native AR C16x16K zero-prefix prefill wave throughput,1M KV; medians retain cold-shape rounds when present, inspect compile events before calling these warm results; not proof of whole-model determinism.')
