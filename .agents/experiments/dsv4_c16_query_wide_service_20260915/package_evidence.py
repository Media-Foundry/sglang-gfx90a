...
for arm in ('A1','B','A2'):
    ...
    assert json.loads((directory/f'P16-wide16k-{arm}.stop.json').read_text())['remaining']==[]
...
manifest=dict(files=len(files),bytes=archive.stat().st_size,
              ...
              scope='Completed C16x16K zero-prefix service ABBA, 1M KV. Inspect summary for drift and performance; not a bitwise-determinism claim.')
