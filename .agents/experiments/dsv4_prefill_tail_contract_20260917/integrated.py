...
flags={
    ...
    'SGLANG_DSV4_PREFILL_MHC_COMMON_SMALL':'1',
...
target=root/'integrated-v1.json';assert not target.exists()
...
source_paths=[Path(__file__),repo/'python/sglang/kernels/ops/layernorm/mhc.py',
    repo/'python/sglang/kernels/ops/layernorm/gfx90a_mhc_prefill_policy.py',
    repo/'python/sglang/srt/layers/dsv4_prefill_tail_policy.py',*paths]
report=dict(status='running',scope=__doc__,pci=pci.value.decode(),flags=flags,cases=[],weight_hashes={},
...
try:
    for layer in (0,20,42):
        ...
        report['weight_hashes'][str(layer)]={name:hashlib.sha256(t.contiguous().view(torch.uint8).cpu().numpy().tobytes()).hexdigest()
            for name,t in [('fn',fn),('scale',scale),('base',base),('norm',norm)]}
        for m in ((1,17,63,64,65,127,128,129,255,256,257,2048,2560,8191,8192) if layer==0 else (64,256,2560)):
            data=[repeat(t,m) for t in seed];data[2]=data[2].view(m,4);data[3]=data[3].view(m,4,4)
            def call(values,hint,large_scope=False,small_scope=False,trace=False):
                tokens=[(v,v.set(large_scope)) for v in (scope._post_reuse,scope._mix_reuse,scope._mix_pair)]
                tokens.append((tail._active,tail._active.set(small_scope)))
                seen=[]
            ...
            selected,selected_paths=call(data,1,small_scope=m<8192,large_scope=m>=8192,trace=True)
            assert all(q['byte_exact'] for q in compare(selected,common)),('selector',layer,m)
            ...
            for mutation in range(3):
                ...
                candidate,_=call(data,1,small_scope=m<8192,large_scope=m>=8192)
                assert all(q['byte_exact'] for q in compare(candidate,b)),('mutation',layer,m,mutation)
                checks.append(dict(mutation=mutation,legacy_vs_common=compare(a,b),selector_exact=True))
            ...
            item=dict(layer=layer,m=m,legacy_paths=legacy_paths,common_paths=common_paths,
                ...
                batch2_common_exact=True,selector_exact=True,selected_paths=selected_paths,checks=checks,samples_ms=samples,
            ...
            del data,legacy,common,selected,batch2,large_data,large,large_prefix,a,b,candidate
