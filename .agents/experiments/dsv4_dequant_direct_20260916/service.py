"""Exact 9.975k owner checkpoint versus direct-row weight expansion."""
import argparse
...
screen=json.loads((root/'rows-screen.json').read_text())
assert screen['status']=='complete'
assert all(c['mutation10_byte_exact'] for c in screen['cases'])
if not checking:
    assert json.loads((root/'check/complete.json').read_text())['live_comparisons']==2752
out=root/args.arm
...
launcher=(root.parent/'dsv4_premix_owner_20260916/B/start-ar-matrix.sh').read_text()
...
flags=('export SGLANG_DSV4_DEBUG_PREFILL_MIX_MFMA=0\n'
       ...
       f'export SGLANG_DSV4_DEBUG_CK_DIRECT_DEQUANT={int(candidate)}\n'
       f'export SGLANG_DSV4_DEBUG_CK_DIRECT_DEQUANT_CHECK={int(checking)}\n')
...
label='P16-direct-dequant-'+args.arm
...
try:
    ...
    for key,value in [('SGLANG_DSV4_C4_PREFILL_QUERY_PRODUCER','0'),
                      ...
                      ('SGLANG_DSV4_DEBUG_CK_DIRECT_DEQUANT',str(int(candidate)))]:
    ...
    exact=re.findall(r'TP(\d+)\] DSV4 direct-row dequant exact: (gate|down)',logs)
    if checking:
        assert Counter(exact)==Counter({(str(r),name):172 for r in range(8) for name in ('gate','down')})
    else:
        assert not exact
    for rank in range(8):
        ...
        assert (f' TP{rank}] DSV4 direct-row CK dequant selected:' in logs)==candidate
