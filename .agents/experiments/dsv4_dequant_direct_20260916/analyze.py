"""Validate direct-row dequant ABBA and unchanged model-output evidence."""
import hashlib
import json
import math
...
assert check['live_comparisons']==2752 and check['diagnostic']
...
for arm in ('A1','B','A2'):
    ...
    assert not json.loads((directory/f'P16-direct-dequant-{arm}.stop.json').read_text())['remaining']
    plans.append(json.loads((directory/'plan.json').read_text()))
    log=(directory/f'P16-direct-dequant-{arm}.service.log').read_text()
    assert 'max_total_num_tokens=1048576' in log
    for rank in range(8):
        assert f'[TP{rank}] pre-mix owner selected:' in log
        assert (f' TP{rank}] DSV4 direct-row CK dequant selected:' in log)==(arm=='B')
...
summary=dict(status='complete',scope='Original V4 TP8 C16x8K, direct-row CK weight expansion over accepted owner baseline, native AR',
    ...
    kv_tokens=1048576,original_weights=True,live_comparisons=2752,quality=quality,
