"""Scalar versus vec4 fixed-order reducer, rebuilt unique CK/H16 common."""
import argparse
import hashlib
...
if not diagnostic:
    assert json.loads((baseline/'check/complete.json').read_text())['all_layers_exact']
out = root/args.arm
...
flags += f'export SGLANG_DSV4_DEBUG_CK_REDUCE_VEC4={int(candidate)}\n'
execute = needle
if diagnostic:
...
label = 'P16-vec4-reduce-'+args.arm
...
try:
    ...
    selected = 'DSV4 unique CK fixed-order vec4 reducer selected'
    if candidate:
