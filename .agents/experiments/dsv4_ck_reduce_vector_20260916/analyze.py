"""Service scalar/vec4 reducer ABBA; reuse prior attention oracle, compare new continuations."""
import collections
import hashlib
...
check = json.loads((baseline / 'check/complete.json').read_text())
assert check['all_layers_exact'] and check['unique_ck']
logpath = baseline / 'check/P16-rebuilt-extras-check.service.log'
...
summary = dict(scope='Scalar versus vec4 fixed-order reducer; identical rebuilt CK/IPC and corrected H16; original V4 TP8 C16x8K',
    ...
    original_weights=True, kv_tokens=1048576, prefill_budget=32768, new_attention_exact_comparisons=0, prior_attention_exact_comparisons=1312,
