"""Isolated native two-bank attention pipeline/K-tile screen, not service rates."""
import hashlib
import json
import os
...
result=dict(scope=__doc__,real_capture=False,rows=8192,status='running',results=[],
...
configs=[('default',16,{}),('stages1',16,{'num_stages':1}),
         ('stages2',16,{'num_stages':2}),('K32',32,{})]
for family in ('c128','c4_varied'):
    ...
    def launch(cfg):
        name,k,kw=cfg;o=outs[name]
        return kernel[(m,1)](q,pk,pi,pp,ek,ei,ep,sink,o,*q.stride(),*pk.stride(),*ek.stride(),
            *o.stride(),8,512,512**-.5,BLOCK_H=16,BLOCK_D=512,BLOCK_K=k,num_warps=1,**kw)
    baseline=configs[0]
    for cfg in configs[1:]:
        launch(baseline);launch(cfg);torch.cuda.synchronize()
        checks=[]
        for mutation in range(5):
            if mutation:
                q.mul_(1.005);pk.mul_(.999);ek.mul_(1.001);sink.add_(.01)
            if mutation==2:
                # Keep pointers/causal schedule; mutate selected occurrences,
                # including invalid sentinels. No silent set deduplication.
                pi[::97]=-1;ei[::113]=-1
            launch(baseline);launch(cfg)
            a,b=outs['default'],outs[cfg[0]]
        ...
        for cycle in range(3):
            for label,c in [('A',baseline),('B',cfg),('B',cfg),('A',baseline)]:
                samples[label].append(measure(lambda:launch(c)))
        resources={}
        for label,c in [('A',baseline),('B',cfg)]:
            code=launch(c)
            resources[label]=dict(registers=code.n_regs,spills=code.n_spills,lds=code.metadata.shared,
                num_stages=code.metadata.num_stages,hsaco_sha256=hashlib.sha256(code.asm['hsaco']).hexdigest())
        med={k:statistics.median(v) for k,v in samples.items()}
        result['results'].append(dict(family=family,candidate=cfg[0],checks=checks,
            samples_ms=samples,median_ms=med,rate_gain_pct=100*(med['A']/med['B']-1),resources=resources))
        save();print(family,cfg[0],med,resources,flush=True)
