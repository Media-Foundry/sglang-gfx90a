"""Synthetic eight-rank wide-indexer oracle, NOT a service speed result.

No production guards are modified. Call the existing runtime-M kernel directly
for small owner batches, preserve query16 groups, exchange logical IDs only.
Include GPU packing, logits, Top-K, RCCL and reconstruction in candidate timing.
Full Q production and compressor are identical/excluded from both timings.
"""
import argparse
import datetime
import hashlib
...
from sglang.kernels.ops.attention.dsv4.gfx90a_indexer_owner import host_plan, reconstruct
...
def main():
    ...
    try:
        for name, width, ext, prefix in [
            ...
            def candidate():
                pq = q.view(torch.uint8).index_select(0, rowids)
                pw = w.index_select(0, rowids)
                pl = lengths.index_select(0, rowids)*valid
                pp = pages.index_select(0, rowids)
                scores = torch.empty((cap,width), device='cuda', dtype=torch.float32)
                reuse_runtime_m[(triton.cdiv(cap,16),triton.cdiv(width,16))](
                    pq,cache,pw,pl,pp,scores,cap,width,np,pp.stride(0),16,16,shuffle,
                    tl.float16 if fp16 else tl.bfloat16,
                ...
                reconstruct[(m,)](gathered,inv,lengths,pages,raw,phys,pages.stride(0),True,num_warps=4)
                return scores
            ...
            for mutation in range(3):
                ...
                assert torch.equal(a.index_select(0,rowids)[valid.bool()].view(torch.int32),b[valid.bool()].view(torch.int32))
