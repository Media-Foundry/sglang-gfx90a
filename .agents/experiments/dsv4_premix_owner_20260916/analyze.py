"""Validate complete owner ABBA and teacher-forced evidence, without null inflation."""
import hashlib
import json
import math
...
assert check['live_comparisons']==2720 and check['diagnostic']
...
for arm in ('A1','B','A2'):
    ...
    assert not json.loads((directory/f'P16-mix-owner-{arm}.stop.json').read_text())['remaining']
    plans.append(json.loads((directory/'plan.json').read_text()))
    log=(directory/f'P16-mix-owner-{arm}.service.log').read_text()
    assert 'max_total_num_tokens=1048576' in log
    for rank in range(8):
        assert (f'[TP{rank}] pre-mix owner selected:' in log)==(arm=='B')
...
summary=dict(status='complete',scope='Original V4 TP8 C16x8K, exact paired pre-mix ownership, native AR',
    ...
    kv_tokens=1048576,original_weights=True,live_comparisons=2720,quality=quality,
