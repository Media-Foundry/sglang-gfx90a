"""Non-scoring real-service owner-chain breakdown with unchanged arithmetic."""
import argparse
import hashlib
...
parser.add_argument('--length', choices=('8k','32k'), required=True)
args=parser.parse_args()
forwards={'8k':4,'16k':8,'32k':16}[args.length]
out=root/args.length;out.mkdir(exist_ok=False)
checkpoint=root.parent/'dsv4_prefill_mhc_common_20260916'
assert json.loads((checkpoint/'acceptance.json').read_text())['common_fp32_20_large_prefill_validated']
...
source=checkpoint/'B'
fixture={'8k':root.parent/'dsv4_dequant_direct_20260916/B/inputs.json',
         '16k':root.parent/'dsv4_wide_owner_integrated_20260916/B/inputs.json',
         '32k':source/'inputs.json'}[args.length]
manifest=json.loads(fixture.read_text())
expected_tokens={'8k':131069,'16k':262141,'32k':524286}[args.length]
...
label='P'+args.length+'-markers-common';state=life.start(label,0)
try:
    life.ready(state)
    ...
    for wave,name in enumerate(('warmup','trace1','trace2','trace3')):
        ...
        reference_path=root.parent/'dsv4_prefill_length_profile_20260916'/args.length/(name+'-response.json')
        reference={r['meta_info']['id']:r for r in json.loads(reference_path.read_text())}
        assert set(reference)==set(byid)
        for rid in rids:
            assert life.completion_ids(reference[rid])==life.completion_ids(byid[rid]), ('first-token drift',rid)
        expected=8*forwards*(wave+1);deadline=time.monotonic()+30
    ...
    life.save('complete.json',dict(input_echo_exact=64,frames=8*forwards*4,
        forwards_per_wave=forwards,input_tokens_per_wave=expected_tokens,diagnostic_only=True))
finally:
    life.stop(state)
