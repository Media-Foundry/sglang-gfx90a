...
import torch
import triton
from candidate import premix8_pair
...
files=[fn_path,x_path,Path(__file__),root/'candidate.py',repo/'python/sglang/kernels/ops/layernorm/gfx90a_mhc_premix_reuse8.py']
result=dict(status='running',scope='Captured layer0 residual/Fn expanded to synthetic occupancy; not fresh service activations or E2E.',
...
for m in args.sizes:
    ...
    eps=1e-6
    def control():return premix8[(24,triton.cdiv(m,8))](x,fn,partials,a,m,eps,num_warps=1)
