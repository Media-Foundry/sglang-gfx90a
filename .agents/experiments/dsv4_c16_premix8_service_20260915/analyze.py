...
for arm in arms:
    ...
    assert read(arm+'/plan.json')['mix_group_size']==(8 if arm=='B' else 4)
    for rank in range(8):
        ...
        assert any(f'TP{rank}]' in line and 'prefill mix-reuse4 selected' in line
                   and f'group={8 if arm=="B" else 4}' in line
                   for line in raw.decode(errors='replace').splitlines())
    ...
    for rep in range(2):
        ...
        rids=[f'mix8-{arm}-{rep}-{i}' for i in range(16)];assert set(by_id)==set(rids)
        ids=[]
