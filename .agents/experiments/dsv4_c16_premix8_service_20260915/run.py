...
spec=importlib.util.spec_from_file_location('life_mix8',helper)
...
flags=('export SGLANG_DSV4_C4_PREFILL_QUERY_REUSE4=1\n'
       ...
       f'export SGLANG_DSV4_PREFILL_MIX_GROUP_SIZE={8 if candidate else 4}\n'
...
life.save('plan.json',dict(candidate=candidate,query_group_size=16,runtime_m=True,mix_group_size=8 if candidate else 4,budget=32768,original_weight=True,kv_tokens=1048576,
    input_sha256=hashlib.sha256((out/'inputs.json').read_bytes()).hexdigest(),sources=sources))
state=life.start('P16-mix8-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-mix8-'+args.arm+'.server-info.json')).read_text())
    ...
    assert env['SGLANG_DSV4_PREFILL_MIX_GROUP_SIZE']==str(8 if candidate else 4)
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'mix8-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
        ...
        if name=='warmup':
            ...
            for rank in range(8):
                ...
                assert any(f'TP{rank}]' in line and 'prefill mix-reuse4 selected' in line
                           and f'group={8 if candidate else 4}' in line for line in logs.splitlines())
    ...
    for rep in range(2):
        ...
        rids=[f'mix8-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'mix8-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
