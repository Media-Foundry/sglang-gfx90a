...
import re
import sys
sys.path.insert(0,str(Path(__file__).resolve().parent.parent/'dsv4_c16_final_profile_20260915'))
from profile_labels import index_intervals

p=argparse.ArgumentParser(description=__doc__)
p.add_argument('--label', default='profile')
p.add_argument('--forwards-per-wave', type=int, choices=(4,8,16), required=True)
p.add_argument('--check-only', action='store_true', help='Read-only replay against archived analysis')
...
args=p.parse_args();n=args.forwards_per_wave;assert re.fullmatch(r'[A-Za-z0-9-]+',args.label)
...
assert len(frames) == 8 * n * 4
assert {(f['rank'], f['sequence']) for f in frames} == {(r,s) for r in range(8) for s in range(1,n*4+1)}
...
selected=[]
spread=[]
for seq in range(n+1,n*4+1):  # Entire first wave excluded, not merely first forward.
...
result=dict(scope='Diagnostic spans include synchronization/waits and instrumentation. Select longest rank envelope per forward, then retain all its stages; not independent rank maxima or a synchronized multi-rank critical-path reconstruction.',
            snapshots=len(frames), warmup_frames=8*n,
            event_envelope_ratio_range=[min(f['event_envelope_ratio'] for f in frames if f['sequence']>n),max(f['event_envelope_ratio'] for f in frames if f['sequence']>n)],
...
if len(frames[0]['layers'][0]['ticks'])==64:
    ...
    for wave in range(3):
        totals={};counts={};paths={}
        for selected_frame in selected[wave*n:(wave+1)*n]:
    ...
    details=dict(scope='Nested subdivisions, already included in coarse spans. Same selected rank/forward. Metadata/launch/wait costs remain inside call boundaries. Owner chain includes packing, logits, Top-K, integer all-gather and reconstruction; owner tail marker is not a separate Top-K measurement.',
                 ...
                 waves=detail_waves)
    if args.check_only:
        assert details==json.loads((ROOT/'details-analysis.json').read_text())
    else:
