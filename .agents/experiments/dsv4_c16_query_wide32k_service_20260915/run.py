"""Wide C4, C16 x 32K real-code service ABBA. Default-off; keep 1M KV."""
import argparse
import hashlib
...
args=p.parse_args()
previous=root.parent/'dsv4_c16_query_wide_service_20260915'
summary=json.loads((previous/'summary.json').read_text())
review=json.loads((previous/'quality-review.json').read_text())
assert summary['throughput_gain_percent']>2
assert review['bounded_semantic_smoke_pass'], 'Review16K outputs before expanding scope'
assert all((previous/a/'complete.json').exists() for a in ('A1','B','A2'))
...
spec=importlib.util.spec_from_file_location('life_wide',helper)
life=importlib.util.module_from_spec(spec);spec.loader.exec_module(life)
life.ROOT=life.OLD=out
fixtures=repo/'.agents/experiments/dsv4_c16_query_wide_20260915'
integrated=json.loads((fixtures/'integrated.json').read_text())
assert integrated['status']=='complete' and integrated['integrated']
assert len(integrated['cases'])==3
assert all(c['mutations']==100 and c['score_bits_exact'] and c['logical_physical_exact'] for c in integrated['cases'])
manifest=json.loads((fixtures/'inputs32k/prefill.json').read_text())
validation=json.loads((fixtures/'input-validation.json').read_text())['inputs32k']
assert validation['official_chat_encoding_exact']==16
assert validation['manifest_sha256']==hashlib.sha256((fixtures/'inputs32k/prefill.json').read_bytes()).hexdigest()
assert len(manifest['requests'])==16
assert sum(len(x['input_ids']) for x in manifest['requests'])==524286
life.save('inputs.json',manifest)
launcher=(root/'start-base.sh').read_text()
assert 'CHUNKED_PREFILL_SIZE=32768 MAX_PREFILL_TOKENS=32768' in launcher
flags=('export SGLANG_DSV4_C4_PREFILL_QUERY_REUSE4=1\n'
       'export SGLANG_DSV4_C4_PREFILL_QUERY_GROUP_SIZE=16\n'
       'export SGLANG_DSV4_PREFILL_MIX_GROUP_SIZE=8\n'
       f'export SGLANG_DSV4_C4_PREFILL_QUERY_WIDE={int(candidate)}\n'
...
paths=('python/sglang/kernels/ops/layernorm/gfx90a_mhc_premix_reuse.py',
...
try:
    life.ready(state)
    info=json.loads((out/('P16-wide32k-'+args.arm+'.server-info.json')).read_text())
    ...
    assert env['SGLANG_DSV4_C4_PREFILL_QUERY_REUSE4']=='1'
    ...
    assert env['SGLANG_DSV4_C4_PREFILL_QUERY_WIDE']==str(int(candidate))
    assert env['SGLANG_DSV4_DEBUG_INDEXER_COMPILE_SHAPES']=='0'
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'wide32k-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
    ...
    assert 'prefill wide-query-reuse selected' not in Path(state['log']).read_text()
    ...
    for rep in range(2):
        ...
        rids=[f'wide32k-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'wide32k-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
