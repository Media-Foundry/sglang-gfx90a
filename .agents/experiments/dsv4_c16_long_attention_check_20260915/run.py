...
out=root/'service-v2';out.mkdir(exist_ok=False)
...
assert len(ratios)==43 and set(ratios)=={0,4,128}
...
try:
    ...
    for length,width in ((16,4096),(32,8192)):
        for rep in range(2):
            ...
            for rank in range(8):
                assert {r[1] for r in checks if r[0]==rank}==set(range(len(ratios))),(length,rank,checks)
                assert all(r[2]==ratios[r[1]] for r in checks if r[0]==rank)
                counts=[sum(r[0]==rank and r[1]==layer for r in checks) for layer in range(len(ratios))]
                assert len(set(counts))==1 and counts[0]>0,(length,rank,counts)
