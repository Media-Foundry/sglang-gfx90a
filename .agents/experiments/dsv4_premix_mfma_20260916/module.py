...
def load_cooperative():
    root=Path(__file__).resolve().parent;repo=root.parents[2]
    return load_jit('dsv4_premix_mfma_cooperative_screen',cuda_files=[str(root/'cooperative.cuh')],
        cuda_wrappers=[(name,'sglang::MhcCooperativeScreen::'+name) for name in ('k32','k64','k128','p32','p64','p128','s4','s16')],
        extra_include_paths=[str(root),str(repo/'python/sglang/kernels/jit/csrc')],
        extra_cuda_cflags=['-O3','-fno-fast-math','-ffp-contract=off'])
