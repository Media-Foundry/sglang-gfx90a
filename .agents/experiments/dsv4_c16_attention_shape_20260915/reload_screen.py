...
target=root/'reload.json';assert not target.exists()
...
result['candidate_sha256']=hashlib.sha256((root/'reload_q.py').read_bytes()).hexdigest()
...
from reload_q import reload_q_kernel
configs=[('H16W1',16,1),('reloadQ',16,1)]
for family in ('c128','c4_varied'):
    ...
    def launch(config):
        ...
        impl=kernel if name=='H16W1' else reload_q_kernel
        return impl[(m,1)](q,pk,pi,pp,ek,ei,ep,sink,o,*q.stride(),*pk.stride(),*ek.stride(),
