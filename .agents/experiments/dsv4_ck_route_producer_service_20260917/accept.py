"""Fail closed on route producer's numerical, dispatch, lifecycle and ABBA evidence.

The spread check is a conservative screening rule, not a confidence interval.
Acceptance is restricted to the measured C16x8K profile, not a global default.
"""
import hashlib
import json
import math
...
def validate_summary(summary):
    ...
    assert summary['live_comparisons'] == 1376
    ...
    for arm, paths in summary['timing_paths'].items():
        ...
        assert paths['k32_ranks'] == ranks
        assert paths['route_ranks'] == (ranks if arm == 'B' else [])
    legs = summary['legs']
...
def main():
    ...
    record = dict(status='accepted_explicit_8k_profile', numerical_exact_on_tested_inputs=True,
                  scope=summary['scope'], default_promoted=False,
                  service_16k_32k_regression_validated=False,
