"""Validate route producer against the current accepted K32 checkpoint."""
import hashlib
import json
import math
...
assert check['live_comparisons']==1376 and check['diagnostic']
...
for arm in ('A1','B','A2'):
    ...
    log=(directory/f'P8-route-producer-{arm}.service.log').read_text()
    ...
    for leg in done['progress']:
        if leg['leg']!='warmup':
            ...
            for wave in measured['rounds']:
                assert wave['total_prompt_tokens']==131069
                ...
                assert math.isclose(131069/seconds,wave['aggregate_input_tok_s'],rel_tol=1e-12)
...
teacher_checks=[]
legacy=root.parent/'dsv4_owner_k32_regression_20260916/8k-B/teacher-forced.json'
legacy_byid={r['meta_info']['id']:r for r in json.loads(legacy.read_text())}
teachers['prior_accepted']=[legacy_byid[f'teacher-B-{i}'] for i in range(16)]
...
paths={arm:json.loads((root/arm/'timing-paths.json').read_text()) for arm in ('A1','B','A2')}
for arm, path in paths.items():
    assert path['k32_ranks']==list(map(str,range(8)))
    assert path['route_ranks']==(list(map(str,range(8))) if arm=='B' else [])
    assert path['premix_owner_ranks']==list(map(str,range(8))) and not path['legacy_splitk']
summary=dict(timing_paths=paths,status='complete',scope='Original V4 TP8 C16x8K CK route producer off vs on, K32 fixed, common FP32/20 MHC, original weights/native AR/1M KV',
