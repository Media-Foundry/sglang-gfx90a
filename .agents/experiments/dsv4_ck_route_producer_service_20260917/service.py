"""Real8K current K32 profile versus opt-in route producer;1M KV fixed."""
import argparse
...
screen=json.loads((root.parent/'dsv4_ck_route_producer_20260916/integrated-v2.json').read_text())
assert screen['status']=='complete' and len(screen['cases'])==6
assert all(all(c['output_byte_exact'] for c in case['checks']) for case in screen['cases'])
if not checking:
    assert json.loads((root/'check/complete.json').read_text())['live_comparisons'] == 1376
...
prior=root.parent/'dsv4_owner_k32_regression_20260916/8k-B'
manifest_path=prior/'inputs.json'
archive=json.loads((prior.parent/'archive-manifest.json').read_text())
entry=next(f for f in archive['files'] if f['path']=='8k-B/inputs.json')
...
label='P8-route-producer-'+args.arm+args.attempt
...
try:
    ...
    if not checking:
        ...
        reference=json.loads((root.parent/'dsv4_owner_k32_regression_20260916/8k-A1/quality-0.json').read_text())
    ...
    if checking:
        ...
        assert len(exact)==1376,len(exact)
        assert all(16384<=int(r[1])<=36864 for r in exact),exact
