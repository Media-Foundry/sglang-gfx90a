...
spec=importlib.util.spec_from_file_location('life_qrunm',helper)
...
flags=('export SGLANG_DSV4_C4_PREFILL_QUERY_REUSE4=1\n'
       ...
       f'export SGLANG_DSV4_C4_PREFILL_QUERY_RUNTIME_M={int(candidate)}\n'
...
paths=('python/sglang/kernels/ops/attention/dsv4/gfx90a_indexer_runtime_m.py',
...
life.save('plan.json',dict(candidate=candidate,query_group_size=16,runtime_m=candidate,budget=32768,original_weight=True,kv_tokens=1048576,
    input_sha256=hashlib.sha256((out/'inputs.json').read_bytes()).hexdigest(),sources=sources))
state=life.start('P16-qrunm-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-qrunm-'+args.arm+'.server-info.json')).read_text())
    ...
    assert env['SGLANG_DSV4_C4_PREFILL_QUERY_RUNTIME_M']==str(int(candidate))
    assert env['SGLANG_DSV4_DEBUG_INDEXER_COMPILE_SHAPES']=='1'
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'qrunm-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
        ...
        if name=='warmup':
            ...
            for rank in range(8):
                assert any(f'TP{rank}]' in line and 'prefill query-reuse4 selected' in line
                           and 'query_group=16' in line and f'runtime_m={int(candidate)}' in line
    ...
    for rep in range(2):
        ...
        rids=[f'qrunm-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'qrunm-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
