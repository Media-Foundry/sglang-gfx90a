"""C16 real 16K prefill: full wide-query versus integrated wide owner.

Exact 10.206k 8K checkpoint retained in both arms. Query producer unchanged;
...
assert len(manifest['requests'])==16 and sum(len(r['input_ids']) for r in manifest['requests'])==262141
...
try:
    ...
    if checking:
        ...
        assert all(int(r[3])==4096 for r in exact),exact
