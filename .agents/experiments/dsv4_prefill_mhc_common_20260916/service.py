"""32K common FP32/20 prefill: full paired reference versus TP row owner.

Both arms use the common large-prefill arithmetic and wide indexer ownership.
Legacy FP16/8 is not an exact reference. Decode/small prefill remain unchanged.
"""
import argparse
...
screen=json.loads((root/'oracle-v2.json').read_text())
...
prior=root.parent/'dsv4_wide_owner_32k_20260916/B'
...
try:
    ...
    assert mix_owner_ranks==(list(map(str,range(8))) if candidate else []),mix_owner_ranks
    answers=[]
    if not checking:
        ...
        reference=json.loads((root.parent/'dsv4_wide_owner_32k_20260916/A1/quality-0.json').read_text())
    ...
    for rank in range(8):
        ...
        assert any(int(w)>2048 for w in hits),(rank,hits)
