"""Validate common FP32/20 large-prefill reference versus row-owned pre-mix."""
import hashlib
import json
import math
...
for arm in ('A1','B','A2'):
    ...
    assert not json.loads((directory/f'P32-common-mhc-{arm}.stop.json').read_text())['remaining']
    plans.append(json.loads((directory/'plan.json').read_text()))
    log=(directory/f'P32-common-mhc-{arm}.service.log').read_text()
    ...
    for leg in done['progress']:
        if leg['leg']!='warmup':
            ...
            for wave in measured['rounds']:
                assert wave['total_prompt_tokens']==524286
                ...
                assert math.isclose(524286/seconds,wave['aggregate_input_tok_s'],rel_tol=1e-12)
...
teacher_checks=[]
legacy=root.parent/'dsv4_wide_owner_32k_20260916/A1/teacher-forced.json'
legacy_byid={r['meta_info']['id']:r for r in json.loads(legacy.read_text())}
teachers['legacy']=[legacy_byid[f'teacher-A1-{i}'] for i in range(16)]
for lhs,rhs in [('A1','A2'),('A1','B'),('legacy','B')]:
...
summary=dict(timing_paths=paths,status='complete',scope='Original V4 TP8 C16x32K, common FP32/20 large-prefill full paired versus owner, native AR',
