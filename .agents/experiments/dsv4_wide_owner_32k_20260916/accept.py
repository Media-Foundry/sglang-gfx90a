...
assert summary['live_comparisons']==21*16*8
...
for arm in ('check','A1','B','A2'):
    ...
    assert not json.loads((root/arm/f'P32-wide-owner-{arm}.stop.json').read_text())['remaining']
record=dict(status='validated_indexer_delta_over_legacy_32k_mhc',
    summary_sha256=hashlib.sha256((root/'summary.json').read_bytes()).hexdigest(),
    launcher_sha256=hashlib.sha256((root/'measured-legacy-launcher.sh').read_bytes()).hexdigest(),
    scope='Original V4 TP8 C16x32K, original checkpoint/native AR, 1M logical KV',
    default_promoted=False, service_32k_indexer_validated=True,
    common_fp32_20_mhc_validated=False, legacy_mhc=dict(fn_dtype="FP16",sinkhorn_iterations=8),
