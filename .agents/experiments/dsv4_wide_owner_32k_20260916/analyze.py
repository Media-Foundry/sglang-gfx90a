"""Validate C16 real 32K wide-owner ABBA and model-output evidence."""
import hashlib
import json
import math
...
for arm in ('A1','B','A2'):
    ...
    assert not json.loads((directory/f'P32-wide-owner-{arm}.stop.json').read_text())['remaining']
    plans.append(json.loads((directory/'plan.json').read_text()))
    log=(directory/f'P32-wide-owner-{arm}.service.log').read_text()
...
summary=dict(timing_paths=paths,status='complete',scope='Original V4 TP8 C16x32K, wide-owner versus full wide-query16; same flags/batch1 legacy MHC common, native AR',
