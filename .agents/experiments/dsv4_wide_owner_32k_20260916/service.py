"""C16 real 32K prefill: full wide-query versus integrated wide owner.

Same startup settings in both arms; batch1 legacy MHC dispatch is recorded.
Query producer unchanged;
wide-query16 full scoring enabled in both, only wide ownership differs.
"""
import argparse
...
screen=json.loads((root.parent/'dsv4_wide_owner_integrated_20260916/oracle.json').read_text())
assert screen['status']=='complete'
if not checking:
    assert json.loads((root/'check/complete.json').read_text())['live_comparisons'] > 0
out=root/args.arm
...
prior=root.parent/'dsv4_dequant_direct_20260916/B'
fixtures=root.parent/'dsv4_c16_query_wide_20260915'
manifest_path=fixtures/'inputs32k/prefill.json'
...
flags=('export SGLANG_DSV4_DEBUG_PREFILL_MIX_MFMA=0\n'
       ...
       f'export SGLANG_DSV4_C4_PREFILL_QUERY_OWNER_WIDE={int(candidate)}\n'
       f'export SGLANG_DSV4_DEBUG_PREFILL_OWNER_CHECK={int(checking)}\n')
...
try:
    ...
    for key,value in [('SGLANG_DSV4_C4_PREFILL_QUERY_PRODUCER','0'),
                      ...
                      ('SGLANG_DSV4_C4_PREFILL_QUERY_OWNER_WIDE',str(int(candidate)))]:
    ...
    life.save('timing-paths.json',dict(legacy_splitk=legacy,premix_owner_ranks=mix_owner_ranks))
    assert {x[0] for x in legacy}==set(map(str,range(8))),legacy
    assert all(x[1]=='fused_tail' and x[3]=='1' and x[4]=='torch.float16' for x in legacy),legacy
    answers=[]
    if not checking:
        ...
        reference=json.loads((root/'A1/quality-0.json').read_text())
    ...
    exact=re.findall(r'\[TP(\d+)\] wide-owner exact: rows=(\d+) local_rows=(\d+) width=(\d+)',logs)
    if checking:
        assert {r[0] for r in exact}==set(map(str,range(8)))
        assert all(int(r[3])==8192 for r in exact),exact
    ...
    for rank in range(8):
        ...
        hits=re.findall(rf'\[TP{rank}\] prefill query-owner selected: rows=\d+ local_rows=\d+ width=(\d+)',logs)
        assert any(int(w)>2048 for w in hits)==candidate,(rank,hits)
