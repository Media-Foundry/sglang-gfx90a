"""One synchronized live-object query producer oracle, then owned shutdown."""
import hashlib
...
paths=set(json.loads((source/'plan.json').read_text())['sources'])|{
    ...
    'python/sglang/kernels/ops/debug/dsv4_owner_producer.py',
...
label='P16-owner-producer-oracle';state=life.start(label,0)
try:
    life.ready(state)
    env=life.owned(state).environ()
    assert env['SGLANG_DSV4_C4_PREFILL_QUERY_OWNER']=='1'
    assert env['SGLANG_DSV4_DEBUG_OWNER_PRODUCER_DIR']==str(data)
    ...
    rids=[f'owner-producer-{i}' for i in range(16)]
    payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
        cache_salt=[f'owner-producer-{i}-{time.time_ns()}' for i in range(16)],
    ...
    assert json.loads((data/'report.json').read_text())['status']=='complete'
    ...
    answer=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],cache_salt=f'owner-producer-france-{time.time_ns()}',
