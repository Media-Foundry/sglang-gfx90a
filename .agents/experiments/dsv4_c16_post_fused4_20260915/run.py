"""Original V4 TP8 C16 post-reuse ABBA arm; production precision, 1M KV."""
import argparse
import hashlib
...
spec=importlib.util.spec_from_file_location('life_post4',helper)
life=importlib.util.module_from_spec(spec);spec.loader.exec_module(life)
life.ROOT=life.OLD=out
old=repo/'.agents/experiments/dsv4_tp8_c16_empty_tiles_20260914/B'
...
flags=(f'export SGLANG_DSV4_PREFILL_POST_FUSED4={int(candidate)}\n'
...
paths=('python/sglang/kernels/ops/layernorm/mhc.py',
...
state=life.start('P16-post4-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-post4-'+args.arm+'.server-info.json')).read_text())
    ...
    assert env['SGLANG_DSV4_PREFILL_POST_FUSED4']==str(int(candidate))
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'post4-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
    ...
    assert 'prefill post-fused4 selected' not in Path(state['log']).read_text()
    ...
    for name,rounds in [('warmup',1),*[(leg,3) for leg in legs]]:
        ...
        if name=='warmup':
            ...
            hit='prefill post-fused4 selected' in logs
            assert hit==candidate,('unexpected selector hit',hit,candidate)
            if candidate:
                for rank in range(8):
                    assert any(f'TP{rank}]' in line and 'prefill post-fused4 selected' in line for line in logs.splitlines())
    ...
    for rep in range(2):
        ...
        rids=[f'post4-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'post4-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
