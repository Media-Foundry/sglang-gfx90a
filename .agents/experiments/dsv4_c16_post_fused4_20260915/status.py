...
for arm in ('A1','B','A2'):
    directory=root/arm
    path=directory/f'P16-post4-{arm}.state.json'
    if not path.exists():
    ...
    if live:
        ...
        result['post_reuse_env']=process.environ().get('SGLANG_DSV4_PREFILL_POST_FUSED4')
    stop=directory/f'P16-post4-{arm}.stop.json'
