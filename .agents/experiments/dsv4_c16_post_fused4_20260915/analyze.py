"""Validate the complete post-reuse ABBA, keeping local and E2E correctness distinct."""
import collections
import hashlib
import json
...
for arm in arms:
    ...
    if arm=='B':
        for rank in range(8):
            assert any(f'TP{rank}]' in line and 'prefill post-fused4 selected' in line
                       for line in raw.decode(errors='replace').splitlines())
    else:assert b'prefill post-fused4 selected' not in raw
    ...
    for rep in range(2):
        ...
        rids=[f'post4-{arm}-{rep}-{i}' for i in range(16)];assert set(by_id)==set(rids)
        ids=[]
...
for a,b in [('A1','A2'),('A1','B'),('A2','B')]:
    changed=[]
    for case,(left,right) in enumerate(zip(outputs[a][0],outputs[b][0],strict=True)):
...
result=dict(legs=legs,quality=quality,sources=sources,first_quality_wave_cross_arm_exact=comparisons,
    ...
    scope='Original V4 TP8/native AR C16x8K zero-prefix prefill wave throughput,1M KV; not proof of whole-model determinism.')
