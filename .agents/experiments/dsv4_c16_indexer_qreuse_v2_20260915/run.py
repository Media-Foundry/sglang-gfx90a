...
spec=importlib.util.spec_from_file_location('life_qreuse4v2',helper)
...
state=life.start('P16-qreuse4v2-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-qreuse4v2-'+args.arm+'.server-info.json')).read_text())
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'qreuse4v2-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
    ...
    for rep in range(2):
        ...
        rids=[f'qreuse4v2-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'qreuse4v2-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
