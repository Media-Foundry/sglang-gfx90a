"""Non-scoring complete profile of the accepted HIP-post/vec4/H16 checkpoint."""
import hashlib
...
root=Path(__file__).resolve().parent;repo=root.parents[2];out=root/'profile';out.mkdir(exist_ok=False)
...
spec=importlib.util.spec_from_file_location('profile_life',root.parent/'dsv4_tp8_ar_down_consumer_20260914/trial.py')
...
state=life.start('P16-markers-postwave',0)
try:
    life.ready(state)
    info=json.loads((out/'P16-markers-postwave.server-info.json').read_text())
    ...
    assert env['SGLANG_DSV4_DEBUG_H16_CHECK']=='0' and env['SGLANG_DSV4_DEBUG_H16_IPC_MANIFEST']
    assert env['SGLANG_DSV4_DEBUG_PREFILL_POST_WAVE']=='1'
    assert env['SGLANG_DSV4_DEBUG_POST_WAVE_CHECK']=='0'
    assert env['SGLANG_DSV4_DEBUG_CK_REDUCE_VEC4']=='1'
    for wave,name in enumerate(('warmup','trace1','trace2','trace3')):
        ...
        rids=[f'corrected-markers-{name}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'corrected-markers-{name}-{i}-{time.time_ns()}' for i in range(16)],
    ...
    for rank in range(8):
        ...
        assert f' TP{rank}] DSV4 exact HIP post-wave selected' in logs
