"""Current vec4/H16 checkpoint versus exact HIP MHC post; optional full post oracle."""
import argparse
import hashlib
...
if not diagnostic:
    assert json.loads((root/'check/complete.json').read_text())['all_post_exact']
out = root/args.arm
...
flags += f'export SGLANG_DSV4_DEBUG_PREFILL_POST_WAVE={int(candidate)}\n'
flags += f'export SGLANG_DSV4_DEBUG_POST_WAVE_CHECK={int(diagnostic)}\n'
execute = needle
if diagnostic:
...
try:
    ...
    assert env['SGLANG_DSV4_DEBUG_PREFILL_POST_WAVE'] == str(int(candidate))
    assert env['SGLANG_DSV4_DEBUG_POST_WAVE_CHECK'] == str(int(diagnostic))
    ...
    selected = 'DSV4 exact HIP post-wave selected'
    if candidate:
    ...
    exact = re.findall(r'TP(\d+)\] DSV4 post wave exact: rows=(\d+)',logs)
    if diagnostic:
        from collections import Counter
        assert set(int(rank) for rank,rows in exact) == set(range(8))
        assert all(n == 340 for n in Counter(rank for rank,rows in exact).values())
        assert all(8192 <= int(rows) <= 65536 for rank,rows in exact)
    for rank in range(8): assert f'[TP{rank}] H16 peer selected:' in logs
    assert sources == {p:hashlib.sha256((repo/p).read_bytes()).hexdigest() for p in paths}
    life.save('complete.json',dict(progress=progress,all_post_exact=diagnostic,
