"""Current checkpoint versus HIP post ABBA, full real-post oracle and continuations."""
import collections
import hashlib
...
assert check['all_post_exact'] and check['unique_ck']
logpath = root / 'check/P16-post-wave-check.service.log'
entries = re.findall(r'TP(\d+)\] DSV4 post wave exact: rows=(\d+)', logpath.read_text())
assert len(entries) == 2720
assert set(int(r) for r,m in entries) == set(range(8))
assert all(n == 340 for n in collections.Counter(r for r,m in entries).values())
...
summary = dict(scope='Triton fused4 versus exact HIP wave post; vec4 CK/corrected H16 common; original V4 TP8 C16x8K',
    ...
    original_weights=True, kv_tokens=1048576, prefill_budget=32768, post_exact_comparisons=2720,
