"""Frozen real layer1/rank6 CK replay on one otherwise idle GCD. Not E2E."""
import hashlib
import importlib
import json
import os
...
assert (root/'capture/complete.json').exists()
assert os.environ.get('HIP_VISIBLE_DEVICES')=='6', 'Reserve physical GCD6 only'
...
assert manifest['provenance']['layer']==1 and manifest['provenance']['rank']==6
assert manifest['provenance']['prefix_lens']==[8192]
report=dict(diagnostic_only=True,physical_gcd=6,real_capture=manifest['provenance'],full=[],atomic=[],fixed=[])
...
for iteration in range(30):
    ...
    record['row4513']=delta(bf16[4513:4514],reference[1][4513:4514])
