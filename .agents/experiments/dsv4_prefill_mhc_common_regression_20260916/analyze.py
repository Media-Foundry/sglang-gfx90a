"""Reconstruct 8K/16K common-policy regression from actual client records."""
import argparse
import hashlib
import json
import math
...
def arm_dir(arm):
    return root/(args.length+'-'+arm)
target=root/('summary-'+args.length+'.json')
assert not target.exists()
check=json.loads((root.parent/'dsv4_prefill_mhc_common_20260916/check/complete.json').read_text())
assert check['live_comparisons']==10880 and check['diagnostic']
...
for arm in ('A1','B','A2'):
    directory=arm_dir(arm)
    done=json.loads((directory/'complete.json').read_text())
    assert done['france_passed'] and not done['diagnostic']
    assert not json.loads((directory/f'P{args.length}-common-regression-{arm}.stop.json').read_text())['remaining']
    plans.append(json.loads((directory/'plan.json').read_text()))
    log=(directory/f'P{args.length}-common-regression-{arm}.service.log').read_text()
    ...
    for leg in done['progress']:
        if leg['leg']!='warmup':
            ...
            for wave in measured['rounds']:
                assert wave['total_prompt_tokens']==expected_tokens
                ...
                assert math.isclose(expected_tokens/seconds,wave['aggregate_input_tok_s'],rel_tol=1e-12)
...
assert len({hashlib.sha256((arm_dir(a)/'inputs.json').read_bytes()).hexdigest() for a in ('A1','B','A2')})==1
cross=[]
for i in range(16):
...
paths={arm:json.loads((arm_dir(arm)/'timing-paths.json').read_text()) for arm in ('A1','B','A2')}
summary=dict(timing_paths=paths,status='complete',scope=f'Original V4 TP8 C16x{args.length}, best owner configuration common MHC policy off/on, native AR',
    ...
    kv_tokens=1048576,original_weights=True,prior_32k_live_comparisons=check['live_comparisons'],quality=quality,
