"""8K/16K regression: identical best owner configuration, common MHC policy off/on."""
import argparse
...
screen=json.loads((root.parent/'dsv4_prefill_mhc_common_20260916/oracle-v2.json').read_text())
assert screen['status']=='complete'
assert json.loads((root.parent/'dsv4_prefill_mhc_common_20260916/acceptance.json').read_text())['common_fp32_20_large_prefill_validated']
...
flags=('export SGLANG_DSV4_DEBUG_PREFILL_MIX_MFMA=0\n'
       ...
       f'export SGLANG_DSV4_PREFILL_MHC_COMMON_FP32={int(candidate)}\n'
...
label='P'+args.length+'-common-regression-'+args.arm
...
try:
    ...
    for key,value in [('SGLANG_DSV4_C4_PREFILL_QUERY_PRODUCER','0'),
                      ...
                      ('SGLANG_DSV4_PREFILL_MHC_COMMON_FP32',str(int(candidate)))]:
    ...
    for rank in range(8):
        ...
        assert (f' TP{rank}] DSV4 common FP32/20 prefill MHC selected:' in logs)==candidate
