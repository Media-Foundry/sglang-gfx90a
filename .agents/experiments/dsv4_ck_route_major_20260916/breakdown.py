...
parser.add_argument('--output',type=Path,default=root/'breakdown.json')
...
try:
    ...
    for location in ('dsv4_cached_ck_drift_20260915/capture','dsv4_c16_real_ck_replay_20260915/capture-v2'):
        ...
        # Isolated substage events are diagnostic only; the ABBA above is the full chain.
        stage_calls={
            'A_remap':lambda:refmod.remap(ids,nv,remapped,m),
            'A_ck':lambda:ck.stage2(inter.view(m*6,1,256),w2,remapped,se,nv,weights,ref_partial),
            'A_reduce':lambda:refmod.reduce_vec4(ref_partial.view(m,6,4096),ref_out),
            'B_pack':lambda:mod.pack(inter,ids,nv,packed,identity,inverse),
            'B_ck':lambda:ck.stage2(packed.view(capacity,1,256),w2,identity,se,nv,weights,partial),
            'B_reduce':lambda:mod.bf16(partial,inverse,out),
        }
        stage_ms={}
        for name,call in stage_calls.items():
            ...
            for repeat in range(5):
                ...
                end.record();end.synchronize();values.append(start.elapsed_time(end)/5)
            stage_ms[name]=dict(median_ms=statistics.median(values),samples_ms=values)
        report['cases'].append(dict(m=m,fixture=str(fixture),capacity=capacity,valid=valid,
            isolated_stage_ms=stage_ms,
