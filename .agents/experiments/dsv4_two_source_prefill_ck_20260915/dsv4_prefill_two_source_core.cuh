// Experimental derivative of dsv4_unified_sparse_decode_ck.cuh at cb0acfe824.
// Production header and decode dispatch are untouched. Only the split core is used.
// Requires SGLANG_DSV4_CK_SENTINEL_ORACLE; encoded bit30 selects the extend KV bank.
#if !defined(SGLANG_DSV4_CK_SENTINEL_ORACLE)
#error "Two-source prefill oracle requires explicit sentinel repair"
#endif
...
inline constexpr int kMaxDecodeM = 192;
...
struct UnifiedSparseDecodeArgs
{
    ...
    int32_t pool_slots;
    float softmax_scale;
    const ck::bhalf_t* extend_kv = nullptr;
    int32_t extend_pool_slots = 0;
};

inline constexpr int kExtendSourceTag = 1 << 30;
__device__ __forceinline__ bool two_source_valid_slot(const UnifiedSparseDecodeArgs& args, int slot)
{
    const bool extend = (slot & kExtendSourceTag) != 0;
    const int row = slot & (kExtendSourceTag - 1);
    return slot >= 0 && row < (extend ? args.extend_pool_slots : args.pool_slots);
}
__device__ __forceinline__ bf16_t two_source_kv_value(
    const UnifiedSparseDecodeArgs& args, int slot, int d)
{
    if(!two_source_valid_slot(args, slot)) return type_convert<bf16_t>(0.0f);
    const bool extend = (slot & kExtendSourceTag) != 0;
    const int row = slot & (kExtendSourceTag - 1);
    const auto* source = extend ? args.extend_kv : args.unified_kv;
    return bit_cast<bf16_t>(source[static_cast<long>(row) * kHeadDim + d]);
}
...
#pragma unroll
            for(int i = 0; i < kKvElementsPerThread; ++i)
            {
                ...
                kv_tile[key * kKvLdsStride + d] =
                    two_source_kv_value(args, slot, d);
            }
            __syncthreads();
        }
    }
...
#pragma unroll
        ...
        const bool score_key_active = softmax_key < score_valid_keys &&
            two_source_valid_slot(args, score_slot);
