"""Validate the complete premix-reuse ABBA, keeping local and E2E correctness distinct."""
import collections
import hashlib
import json
...
for arm in arms:
    ...
    else:assert b'prefill mix-reuse4 selected' not in raw
    ...
    for rep in range(2):
        ...
        rids=[f'mix4-{arm}-{rep}-{i}' for i in range(16)];assert set(by_id)==set(rids)
        ids=[]
