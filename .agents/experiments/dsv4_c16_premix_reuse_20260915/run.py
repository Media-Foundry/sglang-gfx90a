"""Original V4 TP8 C16 premix-reuse ABBA arm; production precision, 1M KV."""
import argparse
import hashlib
...
spec=importlib.util.spec_from_file_location('life_mix4',helper)
...
flags=(f'export SGLANG_DSV4_PREFILL_MIX_REUSE4={int(candidate)}\n'
...
state=life.start('P16-mix4-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-mix4-'+args.arm+'.server-info.json')).read_text())
    ...
    assert env['SGLANG_DSV4_PREFILL_MIX_REUSE4']==str(int(candidate))
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'mix4-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
    ...
    assert 'prefill mix-reuse4 selected' not in Path(state['log']).read_text()
    ...
    for name,rounds in [('warmup',1),*[(leg,3) for leg in legs]]:
        ...
        if name=='warmup':
            ...
            hit='prefill mix-reuse4 selected' in logs
            assert hit==candidate,('unexpected selector hit',hit,candidate)
            if candidate:
                for rank in range(8):
                    assert any(f'TP{rank}]' in line and 'prefill mix-reuse4 selected' in line for line in logs.splitlines())
    ...
    for rep in range(2):
        ...
        rids=[f'mix4-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'mix4-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
