"""Same teacher fixture, one full-Q checked candidate wave; not a speed run."""
import argparse
import hashlib
...
assert args.arm=='B'
out=root/'B-check';out.mkdir(exist_ok=False)
...
flags=(f'export SGLANG_DSV4_C4_PREFILL_QUERY_PRODUCER={int(candidate)}\n'
       'export SGLANG_DSV4_DEBUG_OWNER_PRODUCER_CHECK=1\n')
...
try:
    ...
    assert env['SGLANG_DSV4_DEBUG_OWNER_PRODUCER_CHECK']=='1'
    ...
    life.save('complete.json',dict(input_echo_exact=16,scored_tokens=4096,numerical_only=True,full_q_check=True))
finally:
    life.stop(state)
