...
export FORWARD_HOOKS_JSON='[{"name":"reduction-precision","target_modules":["model.embed_tokens","model.layers.*.input_layernorm","model.layers.*.post_attention_layernorm","model.layers.*.self_attn","model.layers.*.mlp","model.layers.*.engram","model.norm"],"hook_factory":"scripts.rocm.dsv41_trace_hooks:make_hook","config":{"folder":"/tmp/dsv41-reduction-precision-20260913","label":"reduction","max_calls":5,"max_tensor_bytes":33554432,"capture_parameters":false,"ranks":[0]}}]'
