...
for arm in ARMS:
    assert read(root/arm/'complete.json')['input_echo_exact']==16
    assert read(root/arm/f'P16-all-layer-{arm}.stop.json')['remaining']==[]
...
result=dict(scope=__doc__,same_full_inputs_positions_and_layout=not meta_differences,
    ...
    limitation='First prefill forward only; stage hashes locate a frontier, not an unobserved operation or final logits.')
