...
assert check['stage1_hit'] and check['diagnostic'] and check['input_echo_exact']==16
assert json.loads((root/'check/P16-attn-stage1-check.stop.json').read_text())['remaining']==[]
arms=['A1','B','A2'];start=arms.index(args.resume_from)
for arm in arms[:start]:
    assert (root/arm/'complete.json').exists()
    assert json.loads((root/arm/f'P16-attn-stage1-{arm}.stop.json').read_text())['remaining']==[]
for arm in arms[start:]:
