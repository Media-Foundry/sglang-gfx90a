...
for arm in ['A1','B','A2']:
    ...
    assert complete['diagnostic'] is False and complete['stage1_hit']==(arm=='B')
    assert json.loads((directory/f'P16-attn-stage1-{arm}.stop.json').read_text())['remaining']==[]
    ...
    log=(directory/f'P16-attn-stage1-{arm}.service.log').read_bytes()
    ...
    for leg in (['B1','B2'] if arm=='B' else [arm]):
        ...
        if arm=='B':
            assert set(x[0] for x in stage_hits)==set(range(8)),(leg,stage_hits)
            assert all(8192<=m<=65536 and h==8 and c==0 for _,m,h,c in stage_hits)
        else: assert not stage_hits
        legs[leg]=dict(stage_hits=stage_hits,rates=rates,median=statistics.median(rates),wall_median_s=statistics.median(walls),
...
result=dict(formal_echoes=formal_echoes,total_input_per_wave=131069,source_hashes=sources,legs=legs,
            ...
            caveat='Only native prefill attention pipeline scheduling changes; query producer disabled in both arms. Component/checked-service equality is not global model determinism. Warmup excluded.')
