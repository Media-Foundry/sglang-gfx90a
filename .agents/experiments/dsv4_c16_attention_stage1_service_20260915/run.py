"""One-stage native attention A1/B1/B2/A2; query producer OFF in both arms."""
import argparse
import hashlib
...
gate=json.loads((root.parent/'dsv4_c16_attention_pipeline_20260915/check-strided.json').read_text())
assert gate['status']=='complete' and len(gate['results'])==8
assert sum(r['graph_replays_exact'] for r in gate['results'])==1000
screen=json.loads((root.parent/'dsv4_c16_attention_pipeline_20260915/screen.json').read_text())
assert all(c['bits_exact'] for r in screen['results'] if r['candidate']=='stages1' for c in r['checks'])
...
flags=('export SGLANG_DSV4_C4_PREFILL_QUERY_PRODUCER=0\n'
       f'export SGLANG_DSV4_PREFILL_ATTN_STAGE1={int(candidate)}\n'
       f'export SGLANG_DSV4_DEBUG_PREFILL_ATTN_STAGE1_CHECK={int(diagnostic)}\n'
...
label='P16-attn-stage1-'+args.arm
...
try:
    ...
    for name,value in {'SGLANG_DSV4_C4_PREFILL_QUERY_OWNER':'1',
        ...
        'SGLANG_DSV4_PREFILL_ATTN_STAGE1':str(int(candidate)),
        'SGLANG_DSV4_DEBUG_PREFILL_ATTN_STAGE1_CHECK':str(int(diagnostic)),
    ...
    if candidate:
        ...
        assert 'prefill attention-stage1 selected:' not in logs
    assert sources=={p:hashlib.sha256((repo/p).read_bytes()).hexdigest() for p in paths}
    life.save('complete.json',dict(progress=progress,diagnostic=diagnostic,input_echo_exact=16*len(answers),
        stage1_hit=candidate,quality_repeat_exact=None if diagnostic else
