"""Reconstruct 8K/16K K32 regression from actual client records."""
import argparse
import hashlib
import json
import math
...
check=json.loads((root.parent/'dsv4_owner_k32_20260916/check-v2/complete.json').read_text())
assert check['live_comparisons']==2688 and check['diagnostic']
...
for arm in ('A1','B','A2'):
    ...
    log=(directory/f'P{args.length}-owner-k32-regression-{arm}.service.log').read_text()
...
teacher_checks=[]
prior=root.parent/'dsv4_prefill_mhc_common_regression_20260916'/(args.length+'-B')/'teacher-forced.json'
...
summary=dict(timing_paths=paths,status='complete',scope=f'Original V4 TP8 C16x{args.length}, common FP32/20 MHC fixed, owner K16/K32, native AR',
