"""CTA-order-only pre-mix oracle; all arithmetic and reduction shapes unchanged."""
import hashlib
import json
import os
...
root=Path(__file__).resolve().parent;repo=root.parents[2];target=root/'screen.json';assert not target.exists()
...
report=dict(status='running',scope='Repeated/scaled sampled real layer0 inputs; isolated pre-mix, not live full-M or service throughput',
...
for m in (17,8192,32767):
    ...
    def baseline():return premix8_pair[(12,triton.cdiv(m,8))](hidden,fn,partials,a,m,1e-6,num_warps=1)
    for group in (1,2,4,8,16,64):
        def candidate():return premix_order[(12*triton.cdiv(m,8),)](hidden,fn,partials,b,m,group,triton.cdiv(m,8)%group==0,1e-6,num_warps=1)
