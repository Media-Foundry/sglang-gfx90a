"""Reconstruct16K/32K route-producer regression from actual client records."""
import argparse
import hashlib
import json
import math
...
check=json.loads((root.parent/'dsv4_ck_route_producer_service_20260917/check/complete.json').read_text())
...
for arm in ('A1','B','A2'):
    ...
    log=(directory/f'P{args.length}-route-producer-regression-{arm}.service.log').read_text()
...
summary=dict(timing_paths=paths,status='complete',scope=f'Original V4 TP8 C16x{args.length}, common FP32/20 MHC fixed, K32 fixed, route producer off/on, native AR',
