"""Corrected-model H16 ABBA and repeated-continuation identity, distinct metrics."""
import collections
import hashlib
...
logpath=root/'check/P16-corrected-unique-h16-check.service.log'
log=logpath.read_text()
entries=re.findall(r'\[TP(\d+)\] H16 peer output exact: layer=(\d+) rows=(\d+)',log)
...
for arm in ('A1','B','A2'):
    ...
    assert not json.loads((directory/f'P16-corrected-unique-h16-{arm}.stop.json').read_text())['remaining']
    for leg in done['progress']:
        if leg['leg']=='warmup':continue
        assert len(leg['rates'])==3;results[leg['leg']]=leg
    ...
    quality[arm]=dict(repeated_exact_out_of16=equal,
        all_four_exact_requests=sum(len({tuple(life.completion_ids(w[i])) for w in waves})==1 for i in range(16)))
...
summary=dict(scope='Corrected local sinks, original V4 TP8 C16x8K, fixed-order unique CK on both arms, H8 versus H16',
