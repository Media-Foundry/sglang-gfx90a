"""Corrected sink plus unique-slot CK: H8/H16 C16 ABBA and four continuation waves."""
import argparse
import hashlib
...
flags = (f'export SGLANG_DSV4_DEBUG_CK_UNIQUE_MANIFEST={repo}/.agents/experiments/dsv4_ck_unique_store_20260916/build-5fe5f1d3ff25/manifest.json\n'
...
flags += (f'export SGLANG_DSV4_DEBUG_H16_IPC_MANIFEST={root.parent}/dsv4_h16_service_20260916/ipc-manifest.json\n'
          if candidate else 'unset SGLANG_DSV4_DEBUG_H16_IPC_MANIFEST\n')
execute = needle
if diagnostic:
...
label = 'P16-corrected-unique-h16-'+args.arm
...
try:
    ...
    assert bool(env.get('SGLANG_DSV4_DEBUG_H16_IPC_MANIFEST')) == candidate
    ...
    if candidate:
        ...
        assert 'H16 peer selected:' not in logs
    assert sources == {p:hashlib.sha256((repo/p).read_bytes()).hexdigest() for p in paths}
