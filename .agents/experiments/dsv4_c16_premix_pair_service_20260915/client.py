...
def main() -> None:
    ...
    for rep in range(args.rounds):
        ...
        echoes = [result[0].get("prompt_token_ids") for result in results]
        if echoes != [item["input_ids"] for item in selected]:
            raise RuntimeError("server input IDs differ from frozen manifest")
        ...
        record = {
            ...
            "raw_times": [{"begin": r[1], "first": r[2], "end": r[3]} for r in results],
