"""Original V4 TP8 C16 paired-column pre-mix ABBA; one default-off flag."""
import argparse
import hashlib
...
spec=importlib.util.spec_from_file_location('life_mix_pair',helper)
...
flags=('export SGLANG_DSV4_C4_PREFILL_QUERY_REUSE4=1\n'
       ...
       f'export SGLANG_DSV4_PREFILL_MIX_PAIR_COLUMNS={int(candidate)}\n'
...
paths=('.agents/experiments/dsv4_c16_premix_pair_service_20260915/run.py',
       '.agents/experiments/dsv4_c16_premix_pair_service_20260915/client.py',
       '.agents/experiments/dsv4_tp8_ar_down_consumer_20260914/trial.py',
...
life.save('plan.json',dict(candidate=candidate,query_group_size=16,runtime_m=True,mix_group_size=8,pair_columns=candidate,budget=32768,original_weight=True,kv_tokens=1048576,
    input_sha256=hashlib.sha256((out/'inputs.json').read_bytes()).hexdigest(),sources=sources))
state=life.start('P16-mix-pair-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-mix-pair-'+args.arm+'.server-info.json')).read_text())
    ...
    assert env['SGLANG_DSV4_PREFILL_MIX_PAIR_COLUMNS']==str(int(candidate))
    for flag in ('SGLANG_DSV4_C4_PREFILL_QUERY_WIDE', 'SGLANG_DSV4_PREFILL_MHC_CONFIG_ITERS', 'SGLANG_DSV4_PREFILL_MHC_COMB_REFINE20'):
        assert env[flag]=='0'
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'mix-pair-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
    ...
    for name,rounds in [('warmup',1),*[(leg,3) for leg in legs]]:
        ...
        command=[sys.executable,str(root/'client.py'),
        ...
        if name=='warmup':
            ...
            assert hit,'Missing query reuse hit'
            for rank in range(8):
            ...
            if candidate:
                for rank in range(8):
                    assert any(f'TP{rank}]' in line and 'prefill mix-pair selected:' in line
                               and 'group=8 columns=2' in line for line in logs.splitlines())
            else:
                assert 'prefill mix-pair selected:' not in logs
    ...
    for rep in range(2):
        ...
        rids=[f'mix-pair-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'mix-pair-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
