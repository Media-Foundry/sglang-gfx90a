"""Integrated default-off route selector versus guarded original helper.
...
settings={
    'SGLANG_DSV4_PREFILL_CK_ROUTE_PRODUCER':'1',
    'SGLANG_DSV4_DEBUG_CK_ROUTE_MANIFEST':str(root/'build-v1/route/manifest.json'),
    'SGLANG_DSV4_DEBUG_CK_ROUTE_CHECK':'1',
...
import torch
from sglang.kernels.ops.moe import gfx90a_ck_route_producer as integrated
...
target=root/'integrated-v2.json';assert not target.exists()
...
report=dict(status='running',scope=__doc__,pci=pci.value.decode(),settings=settings,cases=[],
    sources={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in
             [Path(__file__),Path(integrated.__file__),
              root.parents[2]/'python/sglang/kernels/jit/csrc/deepseek_v4/gfx90a_ck_route_producer.cuh',Path(current.__file__)]})
...
def load(manifest):
    ...
    return module
report['component_process_group']='real single-rank Gloo, local rank0; not eight-rank service'
save()
scope=_mix_pair.set(True)  # Standalone scope simulation, not a service-dispatch test.
rendezvous=tempfile.TemporaryDirectory(prefix='dsv4-route-component-')
try:
    init_distributed_environment(world_size=1,rank=0,local_rank=0,backend='gloo',
        distributed_init_method='file://'+str(Path(rendezvous.name)/'store'))
    initialize_model_parallel(tensor_model_parallel_size=1,backend='gloo')
    for m in (8192,16384,32765,32767,32768,36864):
        os.environ['SGLANG_DSV4_DEBUG_CK_ROUTE_CHECK']='1'
        ...
        for name in ('hidden','topk_ids','topk_weights','raw_w13','raw_s13','raw_w2','raw_s2','weight13','weight2'):
            ...
            if name in ('hidden','topk_ids','topk_weights'):
                # Boundary-shape component fixture: repeat real prefix rows
                # only when extending beyond the captured32767 rows.
                if value.shape[0]<m:
                    value=torch.cat((value,value[:m-value.shape[0]]),dim=0)
                value=value[:m]
        ...
        def baseline():
            with integrated.reference_scope():
        ...
        os.environ['SGLANG_DSV4_DEBUG_CK_ROUTE_CHECK']='0'
        ...
        report['cases'].append(dict(m=m,fixture=str(fixture),scales_shuffled=shuffled,
            repeated_prefix_rows=max(0,m-meta['hidden']['shape'][0]),
            checks=checks,expanded_weights_exact=True,median_ms=med,speedup=med['A']/med['B'],
            samples=samples,memory=memory))
        save();print('INTEGRATED ROUTED',m,med,'speedup',med['A']/med['B'],flush=True)
