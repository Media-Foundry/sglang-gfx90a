"""Current TP8 stages1 + wide-query coverage and live precompiled-artifact checks.

Uses the existing persistent cache; NOT a cold-cache service A/B comparison.
"""
import hashlib
...
life.save('plan.json',dict(sources=sources,cache=str(cache),scope=__doc__,
    producer=False,stage1=True,wide=True,cold_cache_control=False))
state=life.start('P16-wide-prewarm',0)
try:
    life.ready(state)
    info=json.loads((out/'P16-wide-prewarm.server-info.json').read_text())
    ...
    expected={'SGLANG_DSV4_C4_PREFILL_QUERY_WIDE':'1','SGLANG_DSV4_C4_PREFILL_QUERY_PRODUCER':'0',
        'SGLANG_DSV4_PREFILL_ATTN_STAGE1':'1','SGLANG_DSV4_DEBUG_PREFILL_ATTN_STAGE1_CHECK':'0',
    ...
    for length,width in ((16,4096),(32,8192)):
        for phase,rounds in (('first',1),('warm',3)):
            name=f'{length}k-{phase}'
            assert sources=={p:hashlib.sha256((repo/p).read_bytes()).hexdigest() for p in paths}
            life.resources(name+'-before',life.owned(state));begin=Path(state['log']).stat().st_size
            cmd=[sys.executable,str(client),'--base-url',life.URL,'--inputs',str(out/f'inputs{length}k.json'),
                 '--request-count','16','--rounds',str(rounds),'--tokens','1','--output',str(out/f'{name}.json')]
            ...
            assert all(r['input_echo_exact'] and r['cached_tokens']==[0]*16 and r['completion_lengths']==[1]*16 for r in data['rounds'])
            end=Path(state['log']).stat().st_size
            segment=Path(state['log']).read_bytes()[begin:end].decode(errors='replace')
            pattern=r'\[TP(\d+)\] DSV4 indexer compile-shape rows=(\d+) width=(\d+) pages=(\d+) page_stride=(\d+) group=(\d+) runtime_m=(\d+) align=([0-9,]+) artifact=(\w+)'
            hits=[dict(rank=int(a),m=int(b),width=int(c),pages=int(d),stride=int(e),group=int(f),runtime=int(g),align=h,artifact=i)
                  for a,b,c,d,e,f,g,h,i in re.findall(pattern,segment)]
            if phase=='first':
                relevant=[r for r in hits if r['width']==width]
                assert {r['rank'] for r in relevant}==set(range(8)),(name,hits)
                assert all(r['artifact'] in artifacts and r['group']==16 and r['runtime']==1 for r in relevant),(name,relevant)
            record=dict(name=name,median=data['median_input_tok_s'],rates=[r['aggregate_input_tok_s'] for r in data['rounds']],
                        hits=hits,log_start=begin,log_end=end)
        ...
        for rep in range(2):
            ...
            print('QUALITY',length,rep,'16 exact input echoes',flush=True)
    assert sources=={p:hashlib.sha256((repo/p).read_bytes()).hexdigest() for p in paths}
    life.save('complete.json',dict(progress=progress,quality_echoes=64,first_warm_echoes=128,cold_cache_control=False))
