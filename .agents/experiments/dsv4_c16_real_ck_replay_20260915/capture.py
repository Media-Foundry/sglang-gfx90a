...
out=root/'capture-v2'; out.mkdir(exist_ok=False); data=out/'fixture'
...
label='P16-real-ck-fixture';state=life.start(label,0)
try:
    life.ready(state)
    ...
    rids=[f'real-ck-{i}' for i in range(16)]
    payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
        cache_salt=[f'real-ck-{i}-{time.time_ns()}' for i in range(16)],
