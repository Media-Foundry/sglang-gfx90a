...
spec=importlib.util.spec_from_file_location('life_qgroup16',helper)
...
flags=('export SGLANG_DSV4_C4_PREFILL_QUERY_REUSE4=1\n'
       f'export SGLANG_DSV4_C4_PREFILL_QUERY_GROUP_SIZE={16 if candidate else 4}\n'
...
life.save('plan.json',dict(candidate=candidate,query_group_size=16 if candidate else 4,budget=32768,original_weight=True,kv_tokens=1048576,
    input_sha256=hashlib.sha256((out/'inputs.json').read_bytes()).hexdigest(),sources=sources))
state=life.start('P16-qgroup16-'+args.arm,0)
try:
    life.ready(state)
    info=json.loads((out/('P16-qgroup16-'+args.arm+'.server-info.json')).read_text())
    ...
    assert env['SGLANG_DSV4_C4_PREFILL_QUERY_GROUP_SIZE']==str(16 if candidate else 4)
    ...
    result=life.post(life.URL+'/generate',dict(input_ids=france['input_ids'],
        cache_salt=f'qgroup16-france-{time.time_ns()}',sampling_params=dict(temperature=0,max_new_tokens=32)),600)
        ...
        if name=='warmup':
            ...
            for rank in range(8):
                assert any(f'TP{rank}]' in line and 'prefill query-reuse4 selected' in line
                           and f'query_group={16 if candidate else 4}' in line
    ...
    for rep in range(2):
        ...
        rids=[f'qgroup16-{args.arm}-{rep}-{i}' for i in range(16)]
        payload=dict(input_ids=[r['input_ids'] for r in manifest['requests']],rid=rids,
            cache_salt=[f'qgroup16-quality-{rep}-{i}-{time.time_ns()}' for i in range(16)],
