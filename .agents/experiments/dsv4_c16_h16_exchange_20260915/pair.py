"""Two actual GCDs: full Q exchange+H16 attention+O return versus local H8.
...
try:
    for family,m,req_len in [('c128',8192,4096),('c4',8192,4096),
        ...
        joined=torch.empty((half,16,512),device='cuda',dtype=q.dtype)
        partial=torch.empty_like(joined)
        received_q=torch.empty((half,8,512),device='cuda',dtype=q.dtype)
        received_o=torch.empty_like(received_q)
        ...
        def candidate():
            send_q=q[other].contiguous()
            exchange(send_q,received_q)
            parts=(q[own],received_q) if rank==0 else (received_q,q[own])
            torch.cat(parts,dim=1,out=joined)
            launch(joined,partial,sinks,rank*half)
            send_o=partial[:,(1-rank)*8:(2-rank)*8].contiguous()
            exchange(send_o,received_o)
            out[own].copy_(partial[:,rank*8:(rank+1)*8])
            out[other].copy_(received_o)
        checks=[]
        ...
        item=dict(family=family,m=m,request_len=req_len,logical_fixture_hashes=hashes,
            ...
            extra_static_buffers_bytes=sum(t.numel()*t.element_size() for t in (joined,partial,received_q,received_o)),
