...
def peer_prefill(
    ...
    q_base = tl.where(h_offs[:, None] < 8, q_ptr, q_peer_ptr)
    q = tl.load(q_base + t * q_stride_t
        + (h_offs[:, None] % 8) * q_stride_h + d_offs[None, :] * q_stride_d,
    ...
    out_base = tl.where(h_offs[:, None] < 8, out_ptr, out_peer_ptr)
    tl.store(out_base + t * out_stride_t
        + (h_offs[:, None] % 8) * out_stride_h + d_offs[None, :] * out_stride_d,
        out, mask=h_mask[:, None] & d_mask[None, :])
