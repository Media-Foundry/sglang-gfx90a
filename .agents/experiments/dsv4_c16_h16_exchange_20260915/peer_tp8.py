...
try:
    for family,m,req_len in [('c128',8192,4096),('c4',8192,4096),
                              ('c128',32768,8192),('c4',32768,8192),('c4',32767,8192)]:
        half=(m+1)//2;local_rows=half if parity==0 else m-half;ratio=128 if family=='c128' else 4
        ...
        hashes=[None]*8;dist.all_gather_object(hashes,digest.hexdigest());assert len(set(hashes))==1
        ...
        sink=torch.linspace(-4,4,16,device='cuda')[parity*8:(parity+1)*8].contiguous()
