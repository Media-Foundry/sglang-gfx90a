...
def canonical_csr(indices,indptr,rows):
    """Only the live CSR span is defined; allocation tails may contain garbage."""
    indptr=np.asarray(indptr,dtype=np.int64)[:rows+1]
    assert len(indptr)==rows+1 and bool(np.all(indptr[1:]>=indptr[:-1]))
    first,last=int(indptr[0]),int(indptr[-1])
    assert 0<=first<=last<=len(indices)
    used,canonical=canonical_slots(indices[first:last])
    return used,canonical,(indptr-first).astype(np.int32)
...
def capture(layer_id,rank,batch,args,output):
    ...
    compact={};checked=dict(args);capacities={}
    for bank,ids in (('unified_kv','kv_indices_prefix'),('kv_extend','kv_indices_extend')):
        ...
        used,canonical,pointers=canonical_csr(raw,args[ptr].detach().cpu().numpy(),m)
        checked[ptr]=torch.from_numpy(pointers).to(q.device)
        compact[ptr]=torch.from_numpy(pointers)
        capacities[ids]=dict(allocated=len(raw),live=len(canonical),unique=len(used))
        if len(used):
    ...
    record=dict(layer=layer_id,rank=rank,rows=m,
        ...
        softmax_scale=float(args['softmax_scale']),canonical_output_byte_exact=True,capacities=capacities,
