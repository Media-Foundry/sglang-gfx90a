"""Default-off, fixed-K-order original-V4 TP8 large-prefill QKV diagnostic."""
import logging
...
def _project(X, W, Y, M):
    ...
    for start in range(4096 // 128):
        ...
        acc = tl.dot(a, b, acc)
    tl.store(Y + m[:, None] * 1536 + n[None, :], acc, m[:, None] < M)


def project(x, weight):
    global _logged
    if (x.ndim != 2 or weight.shape != (1536, 4096)
        ...
        raise ValueError("Stable QKV requires BF16 M8192..36864/K4096/N1536 on one GPU")
    y = torch.empty((x.shape[0], 1536), device=x.device, dtype=x.dtype)
    _project[(triton.cdiv(x.shape[0], 128), 12)](
        x, weight, y, x.shape[0], num_warps=8, num_stages=2)
    if not _logged:
        logging.getLogger(__name__).info(
            "DSV4 diagnostic stable QKV selected: M=%d K4096 N1536 tile128/128/128 waves8",
            x.shape[0])
        _logged = True
    return y
