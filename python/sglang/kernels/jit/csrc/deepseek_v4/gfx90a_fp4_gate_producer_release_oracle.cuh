...
__global__ void __launch_bounds__(kNumWaves * kFp4ExpertWave)
    gfx90a_fp4_gate_producer_release_oracle_kernel(
...
#pragma unroll
    ...
    // Every uniform outer-loop iteration is one of exactly 32 CTA
    // contributions for this expert block: W8 * R2 * 32 == I512.
    // The acq_rel release sequence transitively publishes all intermediate
    // stores. The 32nd contribution publishes a monotonically increasing
    // ready epoch for graph replay without ABA.
    __syncthreads();
    if (tid == 0) {
      const uint32_t old = __scoped_atomic_fetch_add(
          counters + expert_block, 1u, __ATOMIC_ACQ_REL,
          __MEMORY_SCOPE_DEVICE);
      if (old % kCtaTasksPerExpert == kCtaTasksPerExpert - 1) {
        __scoped_atomic_store_n(
            ready + expert_block, old / kCtaTasksPerExpert + 1u,
...
template <uint32_t E, uint32_t M, uint32_t T, uint32_t I, uint32_t K,
          uint32_t kAssignments, uint32_t kRows, uint32_t kNumWaves,
          uint32_t kBlocks, uint32_t kPrepacked = 2>
struct Gfx90aFp4GateProducerReleaseOracle {
