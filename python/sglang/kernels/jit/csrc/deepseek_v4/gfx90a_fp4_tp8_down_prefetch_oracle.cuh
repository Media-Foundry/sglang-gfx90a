...
// 8-lane reduction tree follows the production K256 specialization.  The only change is that
...
__global__ void __launch_bounds__(kNumWaves * kFp4ExpertWave)
  ...
  static_assert(K == 256 && kSubgroupWidth == 8,
                "TP8 isolated oracle preserves subgroup8");
