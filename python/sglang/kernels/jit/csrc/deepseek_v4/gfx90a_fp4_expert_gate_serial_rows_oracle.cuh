...
#pragma unroll
      for (uint32_t assignment = 0; assignment < kAssignments; ++assignment) {
        if (!assignment_valid[assignment]) continue;
        gate_acc[assignment] +=
            __shfl_down(gate_acc[assignment], 32, kFp4ExpertWave);
        up_acc[assignment] +=
            __shfl_down(up_acc[assignment], 32, kFp4ExpertWave);
        gate_acc[assignment] +=
            __shfl_down(gate_acc[assignment], 16, kFp4ExpertWave);
        up_acc[assignment] +=
            __shfl_down(up_acc[assignment], 16, kFp4ExpertWave);
        gate_acc[assignment] =
            gfx90a_fp4_dpp_add<0x108u>(gate_acc[assignment]);
        up_acc[assignment] =
            gfx90a_fp4_dpp_add<0x108u>(up_acc[assignment]);
        gate_acc[assignment] =
            gfx90a_fp4_dpp_add<0x104u>(gate_acc[assignment]);
        up_acc[assignment] =
            gfx90a_fp4_dpp_add<0x104u>(up_acc[assignment]);
        gate_acc[assignment] =
            gfx90a_fp4_dpp_add<0x102u>(gate_acc[assignment]);
        up_acc[assignment] =
            gfx90a_fp4_dpp_add<0x102u>(up_acc[assignment]);
        gate_acc[assignment] =
            gfx90a_fp4_dpp_add<0x101u>(gate_acc[assignment]);
        up_acc[assignment] =
            gfx90a_fp4_dpp_add<0x101u>(up_acc[assignment]);
        if (lane == 0) {
          const float gate = fminf(gate_acc[assignment], limit);
          const float up =
              fmaxf(-limit, fminf(up_acc[assignment], limit));
...
template <uint32_t E, uint32_t M, uint32_t T, uint32_t I, uint32_t K,
          uint32_t kAssignments, uint32_t kNumWaves, uint32_t kBlocks,
          uint32_t kPrepacked = 2>
struct Gfx90aFp4ExpertGateSerialRowsOracle {
