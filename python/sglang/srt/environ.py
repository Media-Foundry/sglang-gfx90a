...
class Envs:
    ...
    # CK MFMA producer plus HIP four-branch consumer for the exact BS32 Qwen
    # hyperconnection mix. Other graph tiers retain their established paths.
    SGLANG_QWEN4_GFX90A_CK_HC_MIX_M32 = EnvBool(False)
    ...
    SGLANG_DSV41_ATTN_INVARIANT = EnvBool(False)
    SGLANG_DSV41_HC_STATS_INVARIANT = EnvBool(False)
    SGLANG_DSV41_ENGRAM_GATE_INVARIANT = EnvBool(False)
    SGLANG_DSV41_ROUTER_INVARIANT = EnvBool(False)
    ...
    # Startup-only single-graph diagnostic; run both down arms in a fixed order.
    SGLANG_DSV4_GFX90A_TP8_M32_DOWN_FIXED_WARMUP = EnvBool(False)
    ...
    SGLANG_DSV4_GFX90A_TP8_M32_WOA_HIPBLASLT = EnvBool(False)
    ...
    # Route early BS32 gamma-three verification through a padded BS33 graph so
    # short answers retain full target MoE while steady BS32 keeps M128 compact.
    SGLANG_DSV4_GFX90A_DSPARK_EARLY_EXACT_MAX_POSITION = EnvInt(0)
    ...
    # DSpark-only C4 counterpart for gamma-three target verification.  Unlike
    ...
    SGLANG_DSV4_GFX90A_TP8_C1_ATTN_WARPS2 = EnvBool(False)
    ...
    # Exact target-verify M128 extension of the same gfx90a CK/MFMA sparse
    # decode kernel. Separate from M64/M96 so gamma-3 can be isolated without
    # changing native AR or the DSpark draft graph.
    SGLANG_DSV4_GFX90A_DSPARK_TP4_M128_CK_SPARSE_DECODE = EnvBool(False)
    ...
    # q_lora.  The default preserves the validated production schedule.
    SGLANG_DSV4_GFX90A_TP4_M32_ATTN_ISSUE_ORDER = EnvInt(0)
