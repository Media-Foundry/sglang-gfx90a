...
class Qwen3_5GatedDeltaNet(nn.Module):
    ...
    def finalize_fused_in_proj(self) -> None:
        """Merge the two decode projections when the CUDA BF16 path supports it.

        Qwen4 calls this hook after loading on every platform.  HIP keeps the
        original independent projections; the fused implementation is a
        CUDA-only launch-count optimization.
        """
...
class Qwen3_5LinearDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # Derived hybrid backbones (including Qwen4Exp) retain the sparse-MoE
        # contract without retaining Qwen3.5's model_type string.
...
class Qwen3_5AttentionDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # Dense MLP for variants without routed experts.
