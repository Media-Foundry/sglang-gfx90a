"""Isolated DeepSeek-V4.1 model runtime.

This module is intentionally separate from the local gfx90a-optimized
``deepseek_v4.py`` implementation until parity and performance are validated.
"""
...
try:
    ...
    # The local gfx90a flash-MLA module predates this SM120-only constant.
    # V4.1 uses the value only when is_sm120_supported() is true; keep a
    # harmless sentinel so the HIP text path can import without a CUDA build.
    SM120_DECODE_MAX_TOKENS = 1 << 30
from sglang.kernels.ops.layernorm.mhc_post_split_h import mhc_post_split_h
from sglang.kernels.ops.quantization.fp8_kernel import (
    sglang_per_token_group_quant_fp8,
)
from sglang.srt.compilation.compilation_config import register_split_op
from sglang.srt.configs.deepseek_v41 import DeepseekV41Config as DeepSeekV4Config
...
try:
    ...
    # Older local branches expose the platform probes directly but do not yet
    # have RuntimeContext.get_platform().  Keep this compatibility layer local
    # to the isolated V4.1 module; the shared runtime API remains untouched.
    from sglang.srt.utils.common import (
        is_blackwell_supported,
        is_sm90_supported,
    )

    class _LocalPlatformFacts:
        @property
    ...
    _LOCAL_PLATFORM_FACTS = _LocalPlatformFacts()

    def get_platform():
        return _LOCAL_PLATFORM_FACTS
...
class DeepseekV41ForCausalLM(nn.Module):
    ...
    def _prewarm_mhc_kernels(self) -> None:
        ...
        if _is_npu or _is_xpu or _is_hip or not envs.SGLANG_OPT_USE_TILELANG_MHC_PRE.get():
            return
        layer = next(
