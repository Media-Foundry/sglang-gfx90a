#!/usr/bin/env python3
"""Oracle for TP4 A4/R2 grouped-down row1 VMEM prefetch."""
...
ASSIGNMENTS, ROWS, WAVES, GATE_BLOCKS, DOWN_BLOCKS, LDS = 4, 2, 8, 2080, 832, 2


@cache_once
def candidate_module():
    args = make_cpp_args(
        E, M, T, N, I, ASSIGNMENTS, WAVES, DOWN_BLOCKS, LDS
    )
    return load_jit(
        "gfx90a_fp4_expert_down_row_prefetch_oracle",
        *args,
        cuda_files=[
...
def main() -> None:
    ...
    gate = _jit_gate_up_grouped(
        E, M, T, I, H, ASSIGNMENTS, ROWS, WAVES, GATE_BLOCKS, LDS
    )
    baseline = _jit_down_grouped(
        E, M, T, N, I, ASSIGNMENTS, ROWS, WAVES, DOWN_BLOCKS, LDS
    )
    ...
    for name, down in (("A", baseline), ("B", candidate)):
        ...
        def quant_stage(state=state):
            q, scale = per_token_group_quant_int8(state["intermediate"], 32)
            state["iq"], state["isc"] = q, scale
    ...
    def assert_exact(label: str):
        for tensor_name in ("intermediate", "partial", "output"):
            ...
            if not torch.equal(a, b):
                ...
                raise RuntimeError(
                    f"{label} {tensor_name} mismatch max_abs={diff}"
                )
