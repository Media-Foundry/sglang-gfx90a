...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.29\textwidth}Y@{}}
    ...
    Hardware & Four visible MI250X GCDs (gfx90a), approximately 64 GiB HBM per GCD \\
...
The deployment uses AMD Instinct MI250X/MI250 gfx90a GCDs. The software environment is fixed to PyTorch \texttt{2.12.0a0+git78258b9} and HIP/ROCm \texttt{7.14.60850}, with local SGLang and AIter source trees. The model at \texttt{/home/pc/models/modelscope} consists of 48 indexed safetensors shards. Residual \texttt{.incomplete} files are not referenced by the index and are not loaded.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item MI250X block-FP8 tuned configurations for M512 and M1024 dense projections;
...
Measurements are rejected when unexpected scheduler, compiler, profiler, or model processes remain on the target GCDs. The native-AR harness rejects nonzero \texttt{SPECULATIVE\_*} variables. The service is bound to NUMA node 1 because node 0 produced repeated uncorrectable host-DRAM MCEs during long prefill tests.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Operations.} Place both the model service and port-30002 proxy under systemd or a supervisor, and investigate the node-0 DIMM/EDAC fault during maintenance.
\end{enumerate}

\section{Conclusion}

The principal result is not a single kernel but a reproducible gfx90a inference workflow built around correctness-first validation, hardware-shape-driven optimization, and endpoint-level ABBA tests. The project explicitly withdrew an attractive but incorrect 60 tok/s result. After W2 recovery, small-M packed-FP4-by-INT8 decode, lower collective latency, and large-M CDNA2 MFMA prefill work, the corrected native AR path reached approximately 74.5 tok/s and long prefill reached approximately 2.23k input tok/s. The evidence indicates that MI250X is limited less by nominal aggregate HBM bandwidth than by the mismatch between stored FP4 and CDNA2 execution formats, low-M utilization, and fixed synchronization costs per layer.
