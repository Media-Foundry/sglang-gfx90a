...
\usepackage{fontspec}
% Keep the local Arial rendering when available, while allowing arXiv's
% XeLaTeX environment to compile without proprietary system fonts.
...
The experimental host is a Supermicro A+ Server AS-4124GQ-TNMI with two AMD EPYC 7763 processors and SMT enabled. Host memory comprises 32 32-GiB DDR4-2933 2Rx8 RDIMMs, for 1 TiB installed capacity. The deployment uses four visible AMD Instinct MI250 gfx90a GCDs. The software environment is fixed to PyTorch \texttt{2.12.0a0+git78258b9} and HIP/ROCm \texttt{7.14.60850}, with local SGLang and AIter source trees. The checkpoint consists of 48 indexed safetensors shards. Residual \texttt{.incomplete} download files are not referenced by the index and are not loaded.
